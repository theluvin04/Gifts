...
import { Disc, Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, ChevronLeft, Music, ListMusic, Sparkles } from 'lucide-react';
...
export const VinylMusicPlayer: React.FC<VinylMusicPlayerProps> = ({ playlist, onBack }) => {
    ...
    >
      ...
      <div className="w-full flex items-center justify-between mb-6">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-2 text-sm font-semibold text-rose-700 bg-white/80 backdrop-blur-md rounded-full shadow-sm border border-rose-200 hover:bg-rose-50 transition cursor-pointer"
        >
      ...
      <div className="relative w-full max-w-md bg-amber-950/90 rounded-3xl p-6 sm:p-8 shadow-2xl border-4 border-amber-900/60 flex flex-col items-center overflow-hidden">
        ...
        <div className="relative w-64 h-64 sm:w-72 sm:h-72 rounded-full bg-stone-900 p-2 shadow-inner border-4 border-amber-700/50 flex items-center justify-center mb-6">
          ...
          <div
            ...
            style={{
              backgroundImage: 'radial-gradient(circle, #1a1a1a 10%, #0a0a0a 15%, #1a1a1a 30%, #0a0a0a 45%, #1a1a1a 60%, #000 70%)',
            }}
          >
          ...
          {/* Floating musical notes while playing */}
          {isPlaying && (
            <div className="absolute -top-4 -left-2 text-rose-300 text-xl animate-bounce pointer-events-none">
              🎵
            </div>
          )}
          {isPlaying && (
            <div className="absolute -bottom-2 -right-2 text-pink-300 text-lg animate-pulse pointer-events-none">
              ✨
            </div>
          )}
        </div>
        ...
        <div className="relative z-10 flex items-center justify-between w-full px-4">
          ...
          <button
            ...
            className="text-amber-200 hover:text-white transition p-2 cursor-pointer"
          >
            {isMuted ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}
          </button>

          {/* Middle playback group */}
          <div className="flex items-center gap-4">
            <button
              onClick={handlePrev}
              className="p-2.5 bg-amber-800/80 hover:bg-amber-700 text-amber-100 rounded-full shadow transition active:scale-95 cursor-pointer"
              title="Bài trước"
            >
            ...
            <button
              onClick={togglePlay}
              className="p-4 bg-gradient-to-r from-rose-500 to-pink-500 hover:from-rose-600 hover:to-pink-600 text-white rounded-full shadow-lg shadow-rose-500/50 transition active:scale-95 cursor-pointer"
          ...
          <div className="flex items-end gap-1 h-5 w-6">
            <div className={`w-1 bg-rose-400 rounded-full ${isPlaying ? 'h-5 animate-pulse' : 'h-1.5'}`} />
            <div className={`w-1 bg-rose-400 rounded-full ${isPlaying ? 'h-3 animate-bounce' : 'h-1.5'}`} />
            <div className={`w-1 bg-rose-400 rounded-full ${isPlaying ? 'h-4 animate-pulse' : 'h-1.5'}`} />
          </div>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showPlaylist && (
          <motion.div
            ...
            className="w-full max-w-md mt-4 bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-xl border border-rose-200 overflow-hidden"
          >
              ...
              {playlist.map((track, idx) => (
                ...
                >
                  <div className="flex items-center gap-3 truncate">
                    <span className="text-xs text-rose-500 w-4">{idx + 1}</span>
