...
export default function App() {
  ...
  useEffect(() => {
    try {
      ...
      // Có thể đầy localStorage nếu người dùng đưa quá nhiều ảnh base64.
      // Draft trong React state vẫn tiếp tục hoạt động trong phiên hiện tại.
    }
  }, [config]);
      ...
      <main className="flex min-h-[100svh] items-center justify-center bg-[#fff9fb] px-5">
        <div className="w-full max-w-sm rounded-[28px] border border-rose-100 bg-white p-7 text-center shadow-xl">
          ...
          <button
            ...
            className="mt-5 rounded-full bg-rose-500 px-5 py-2.5 text-sm font-bold text-white"
          >
            Về trang chủ
          </button>
        </div>
    ...
    <main className="relative flex min-h-screen flex-col justify-between overflow-x-hidden bg-gradient-to-b from-pink-50 via-rose-50 to-pink-100 text-slate-800 selection:bg-pink-300 selection:text-pink-900">
      ...
      <div className="fixed left-4 top-4 z-40">
        <button
          ...
          className="flex items-center gap-1.5 rounded-full border border-rose-200 bg-white/80 px-3 py-2 text-xs font-bold text-rose-700 shadow-sm backdrop-blur-md transition hover:bg-white"
